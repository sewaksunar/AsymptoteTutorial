#!/usr/bin/env python3
"""
Create gear animation using Python (iteration) + Asymptote (figures) + FFmpeg (video)
"""
...
PITCH_RADIUS = 132   # mm
ADDENDUM_RADIUS = PITCH_RADIUS + 12
BASE_RADIUS = PITCH_RADIUS * math.cos(math.radians(PRESSURE_ANGLE))
DEDENDUM_RADIUS = PITCH_RADIUS - 15

# Create output directory
OUTPUT_DIR.mkdir(exist_ok=True)

# Clean up previous frames
for f in OUTPUT_DIR.glob("frame_*.pdf"):
    f.unlink()
for f in OUTPUT_DIR.glob("frame_*.png"):
    f.unlink()

print(f"Generating {FRAMES} animation frames...")

# Generate each frame
for frame_num in range(FRAMES):
    angle = (360.0 / FRAMES) * frame_num
    angle_rad = math.radians(angle)
    
    # Create Asymptote script for this frame
    asy_script = f"""settings.outformat = "pdf";
import fontsize;
import geometry;
...
// ============= FRAME {frame_num} ANIMATION =============

int numTeeth = {NUM_TEETH};
real phi = {PRESSURE_ANGLE}, rp = {PITCH_RADIUS};
...
"""
    ...
    # Write Asymptote script
    asy_file = OUTPUT_DIR / f"frame_{frame_num:03d}.asy"
    with open(asy_file, "w") as f:
        f.write(asy_script)
    
    # Compile with Asymptote (no preview)
    pdf_file = OUTPUT_DIR / f"frame_{frame_num:03d}.pdf"
    print(f"  Frame {frame_num:02d}/{FRAMES-1:02d}: Compiling Asymptote...", end="", flush=True)
    
    result = subprocess.run(
        ["asy", "-f", "pdf", "-noV", "-o", str(pdf_file), str(asy_file)],
        cwd=".",
        capture_output=True,
        text=True
    )
...
print(f"\nConverting PDFs to PNG (150 dpi)...")
subprocess.run(
    ["magick", "-density", "150", str(OUTPUT_DIR / "frame_*.pdf"), str(OUTPUT_DIR / "frame_%03d.png")],
    shell=True,
    capture_output=True
)
...
subprocess.run(
    ["ffmpeg", "-framerate", str(FPS), "-i", str(OUTPUT_DIR / "frame_%03d.png"),
     "-vf", "scale=1200:1200", "-c:v", "libx264", "-pix_fmt", "yuv420p",
     str(OUTPUT_DIR / "gear_animation_final.mp4"), "-y"],
    capture_output=True
)
...
subprocess.run(
    ["ffmpeg", "-framerate", str(FPS), "-i", str(OUTPUT_DIR / "frame_%03d.png"),
     "-vf", "scale=800:800", str(OUTPUT_DIR / "gear_animation_final.gif"), "-y"],
    capture_output=True
)

# Copy outputs to main directory
print(f"\nCopying outputs to Animations folder...")
shutil.copy(OUTPUT_DIR / "gear_animation_final.mp4", "gear_animation_hybrid.mp4")
shutil.copy(OUTPUT_DIR / "gear_animation_final.gif", "gear_animation_hybrid.gif")

print(f"\n✓ Complete!")
print(f"  - gear_animation_hybrid.mp4")
print(f"  - gear_animation_hybrid.gif")
print(f"  - Frame files in: {OUTPUT_DIR}/")
