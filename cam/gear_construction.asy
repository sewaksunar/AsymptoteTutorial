...
// ── Construction lines ───────────────────────────────────────
// Centre line
draw(O2+(-0.30,0) -- O3+(0.30,0), gray(0.55)+0.8bp+dashed);
// Common tangent (vertical through P)
draw((0,-1.20) -- (0,1.20), gray(0.55)+0.8bp+dashed);
...
// Pressure-angle arc
draw(arc(P, 1, -(degrees(phi)-90), 90.0), black+0.9bp, Arrow(3));
label("$\phi$", 1*(sin(phi/2)-0.25, 1*cos(phi/2)),
      fontsize(9)+black);

// Radius labels
label("$R_2$", (-R2/2,  0.09), fontsize(9)+blue);
label("$R_3$", ( R3/2,  0.09), fontsize(9)+blue);
...
// //  Legend
// // ══════════════════════════════════════════════════════════════
// real lx = R3+0.40,  ly = 1.80,  ls = 0.44;
// label("\textbf{Legend}", (lx+ls/2, ly+0.28), fontsize(10));

// void leg(real y, pen p, string s) {
//     draw((lx,y) -- (lx+ls,y), p);
//     label(s, (lx+ls+0.08, y), E, fontsize(8));
// }
// leg(ly-0.00, pitchPen,        "Pitch circle");
// leg(ly-0.28, basePen,         "Base circle");
// leg(ly-0.56, addPen,          "Addendum circle");
// leg(ly-0.84, dedPen,          "Dedendum circle");
// leg(ly-1.12, orange+1.5bp,    "Line of action");
// draw((lx, ly-1.40) -- (lx+ls, ly-1.40), heavyred+2.8bp);
// label("Contact zone $CD$", (lx+ls+0.08, ly-1.40), E, fontsize(8));

// // ══════════════════════════════════════════════════════════════
// //  Title
// // ══════════════════════════════════════════════════════════════
// label("{\large\textbf{Involute Spur-Gear Construction}}",
//       (0.5*(R3-R2), -(Ra3+0.42)), fontsize(11));
// label("$N_2=20,\;\; N_3=40,\;\; P_d=10\;\mathrm{teeth/in},"
//       + "\;\; \phi=20^\circ$",
//       (0.5*(R3-R2), -(Ra3+0.70)), fontsize(9));
...
arrowlabel("$B$", B  +(0.5, 0.1),  B +(0,0),   darkgreen);
arrowlabel("$C$", C  +(-0.88,-0.20),  C +(0,0),   orange);
arrowlabel("$D$",  D  +( 0.5, 0.2),  D +(0,0),    black);

// // Key-point mini-legend
// real px = lx,  py = ly-1.72,  ps = 0.13;
// label("\textbf{Key Points}", (px+ls/2, py+0.05), fontsize(9));
// void ptleg(real y, pen col, real sz, string s) {
//     dot((px+ps/2, y), col+(sz*bp));
//     label(s, (px+ps+0.08, y), E, fontsize(7.5));
// }
// ptleg(py-0.22, red,       9.0, "$P$\;---\;Pitch point");
// ptleg(py-0.44, black,     7.0, "$O_2,O_3$\;---\;Shaft centres");
// ptleg(py-0.66, darkgreen, 7.0, "$A,B$\;---\;Base-circle tangent pts");
// ptleg(py-0.88, orange,    8.0, "$C,D$\;---\;Contact endpoints");
