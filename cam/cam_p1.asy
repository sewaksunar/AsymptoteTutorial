...
// Base circle labels
label("$A$", R*dir(A_ang), dir(A_ang)*1.2);
label("$S$", R*dir(S_ang), dir(S_ang)*1.2);
label("$T$", R*dir(T_ang), dir(T_ang)*1.2);
label("$P$", R*dir(P_ang), dir(P_ang)*1.2);
...
for(int i = 0; i <= n; ++i) {
    ...
    if(i > 0 && i < n)
        label(string(i), pos, dir(ang)*1.2, fontsize(7));
}
...
// 1) Outstroke: smooth curve A -> G (linear follower = straight radial lift)
...
label("$G$", (R+stroke)*dir(S_ang), dir(S_ang)*1.15, fontsize(9));
label("$H$", (R+stroke)*dir(T_ang), dir(T_ang)*1.15, fontsize(9));
